# 🧠 A2LLM — a genuine, from-scratch language model family

**A2LLM** is a family of small decoder-only Transformers (next-token
prediction) built from zero — no API wrappers, no pretrained weights, no
shortcuts. One architecture, six sizes: from an **ESP32 microcontroller**
(34K params, int8) up to a **14.5M-param VPS model**. It trains locally, on
any PC / VPS / Colab, and exports to an int8 engine that runs on an ESP32.
...
```
          ...
          ┌───────────┼───────────┐
          │           │           │
       A2LLM-ESP   A2LLM-Base   A2LLM-Android
          │           │           │
...
│   ├── train.py      # train any preset
│   ├── download_model.py  # download trained checkpoints from Releases
│   ├── prepare_data.py    # tiny_shakespeare
│   ├── prepare_data_large.py  # multilingual corpus (en+hi+ur+bn)
│   ├── generate.py / evaluate.py / export_esp32.py
├── configs/          # tiny.json (ESP32) / default.json (PC)
├── esp32/            # ESP-IDF project + pure-C inference engine
├── data/             # raw corpus downloads + smoke test text
├── checkpoints/      # best.pt / latest.pt per run (from Releases)
├── tests/            # 45 tests incl. the overfit correctness test
├── colab_*.ipynb     # free-T4 training notebooks (nano/micro/mini/small/all)
├── setup_colab.sh    # one-cell Colab setup (free T4 GPU)
├── requirements.txt
└── pyproject.toml    # installs the `a2llm` console command
```
...
| micro  |  120K   |  64 |  64 |   2    | any laptop, seconds |
...
## Trained checkpoints (GitHub Releases)

Every trained model is published as a release asset — download any of them
with one command (no checkout needed):

```bash
python scripts/download_model.py            # list available models
python scripts/download_model.py nano       # ESP32 model (34K, int8-ready)
python scripts/download_model.py micro      # laptop model, multilingual
python scripts/download_model.py mini       # default PC model
```

| Release | Contents | Result |
|---------|----------|--------|
| v0.3.0  | `a2llm-micro.pt` (multilingual) | val_loss 1.5686, ppl 4.80 (200K steps, T4) |
| v0.2.0  | `a2llm-mini.pt` | val_loss 3.3162, ppl 27.56 (4000 steps, T4, 67.6s) |
| v0.1.0  | `a2llm-esp32-nano.pt`, `a2llm-micro.pt` | first trained checkpoints |

Open in Colab and train any tier on a free T4 GPU:
